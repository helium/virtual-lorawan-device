...
async fn run(opt: Opt) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let open_channels = fetch_open_channels().await?;

    println!("{:#?}", open_channels);
    // for state_channel in state_channels {
    //     if let Data::state_channel_open_v1(open_channel) = state_channel {
    //         open_channels.push(open_channel);
    //     }
    // }
    //
    // let pending = open_channels.remove(0);
    // let open = open_channels.remove(0);
    // println!("Pending remaining {}", pending.remaining_blocks(block_height));
    // println!("{:#?}", pending);
    // println!("Open remaining {}", open.remaining_blocks(block_height));
    // println!("{:#?}", open);
