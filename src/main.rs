...
use lorawan_encoding::{
    default_crypto::DefaultFactory as LorawanCrypto,
};
#[cfg(abp)]
use lorawan_encoding::{
    keys::AES128,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(abp)]
        let (lorawan, mut lorawan_receiver, lorawan_sender) =
        {
            ...
            let (radio, mut lorawan_receiver, lorawan_sender) = UdpRadio::new(instant.clone(), mac_address, host).await;
    ...
    #[cfg(not(abp))]
        ...
        {
            // LoRaWAN Device with OTAA Constructor
            let (radio, lorawan_receiver, lorawan_sender) = UdpRadio::new(instant.clone(), mac_address, host).await;
            ...
            let appkey = [0xFB, 0x4B, 0x19, 0xE9, 0xF8, 0xD4, 0xB1, 0x50, 0x35, 0x76, 0xCE, 0x9B, 0xD8, 0x79, 0x9C, 0xD3];
    ...
    #[cfg(not(abp))]
        lorawan_sender.send(IntermediateEvent::NewSession).await.unwrap();
    ...
    tokio::spawn(
        async move {
            loop {
                ...
                lorawan_sender.send(IntermediateEvent::SendPacket(vec![1,2,3,4], fport, true)).await.unwrap();
            }
        }
    );

    loop {
        let event = lorawan_receiver.recv().await.expect("Channel unexpectedly closed");
        let (new_state, response) = {
            match event {
                IntermediateEvent::NewSession => {
                    lorawan
                        .handle_event(LorawanEvent::NewSessionRequest)
                }
